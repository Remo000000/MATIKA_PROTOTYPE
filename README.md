...
Демо, курстық немесе диплом жұмысы, пилот үшін қолайлы. Өндіріске шығармас бұрын қауіпсіздік, пошта және инфрақұрылым бойынша жеке тексеруді жоспарлаған жөн.
...
## Бұлтта орналастыру (тегін тариф, дипломға скриншот)

Төменде **Render** және **Heroku** үшін типтік жол (тарифтер уақыт өте өзгереді — провайдер сайтынан тексеріңіз).

### Render.com

1. Render дашбордында **PostgreSQL** құрыңыз (Free немесе Starter).
2. Осы репозиториймен **Web Service** құрыңыз: **Build command**:  
   `pip install -r requirements.txt && python manage.py collectstatic --noinput`  
   **Start command**:  
   `gunicorn matika.wsgi:application --bind 0.0.0.0:$PORT`
3. **Environment**-те кем дегенде: PostgreSQL панелінен `DATABASE_URL`, `SECRET_KEY`, `DEBUG=0`, `ALLOWED_HOSTS` (`*.onrender.com` немесе өз доменіңіз), `CSRF_TRUSTED_ORIGINS=https://сіздің-сервис.onrender.com`, қажет болса `DEFAULT_ORGANIZATION_SLUG` / `DEFAULT_ORGANIZATION_NAME`.
4. Алғашқы деплойдан кейін **Shell**-де `python manage.py migrate` (немесе release command қосыңыз).
5. Скриншоттар: орта айнымалылары (сырсыз), сәтті деплой, шолғышта кіру беті.

### Heroku

1. [Heroku CLI](https://devcenter.heroku.com/articles/heroku-cli) орнатып, аккаунтқа кіріңіз.
2. `heroku create your-app-name`, содан кейін `heroku addons:create heroku-postgresql:mini` (немесе қолдағы актуалды mini/тегін жоспар).
3. Репозиторийде `Procfile` және `runtime.txt` бар. Конфиг:  
   `heroku config:set DEBUG=0 SECRET_KEY=... ALLOWED_HOSTS=.herokuapp.com CSRF_TRUSTED_ORIGINS=https://your-app-name.herokuapp.com`
4. `git push heroku main` — жинақтан кейін `heroku run python manage.py migrate`.
5. Скриншоттар: **Resources** / **Settings**, `heroku logs --tail`, жұмыс істейтін сайт.

Екі жағдайда да статика **WhiteNoise** арқылы; `MEDIA` үшін сыртқы сақтау (S3-үйлесімді т.б.) қажет болуы мүмкін — оқу демосы үшін көбіне БД және статика жеткілікті.

## Лицензия

Жоба **MIT** лицензиясымен таратылады (`LICENSE` файлы). Қатысу ережелері — `CONTRIBUTING.md`.
